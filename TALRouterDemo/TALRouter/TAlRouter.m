...
NSString *const TAlRouterParameterURL = @"TAlRouterParameterURL";
NSString *const TAlRouterParameterCompletion = @"TAlRouterParameterCompletion";
NSString *const TAlRouterParameterUserInfo = @"TAlRouterParameterUserInfo";
...
@implementation TAlRouter

+ (instancetype)sharedInstance
{
    static TAlRouter *instance = nil;
...
+ (void)registerURLPattern:(NSString *)URLPattern toHandler:(TAlRouterHandler)handler
{
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        TAlRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[TAlRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[TAlRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
{
    TAlRouter *router = [TAlRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL matchExactly:NO];
    TAlRouterObjectHandler handler = parameters[@"block"];
...
+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(TAlRouterObjectHandler)handler
{
    [[self sharedInstance] addURLPattern:URLPattern andObjectHandler:handler];
}

- (void)addURLPattern:(NSString *)URLPattern andHandler:(TAlRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(TAlRouterObjectHandler)handler
{
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url matchExactly:(BOOL)exactly
{
    ...
    parameters[TAlRouterParameterURL] = url;
